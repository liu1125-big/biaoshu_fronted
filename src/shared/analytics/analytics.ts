...
async function migrateLegacyClientId(config: ClientConfig) {
  ...
  try {
    const result = await apiClient.config.save(migratedConfig);
    if (result?.success) {
      removeLegacyClientId();
...
function getVersion() {
  if (!versionPromise) {
    versionPromise = Promise.resolve(import.meta.env.VITE_APP_VERSION || '0.1.0');
  }
